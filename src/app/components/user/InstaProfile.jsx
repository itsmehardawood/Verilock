...
function ProfileDetailsModal({ isOpen, profile, onClose, onTakedownRequest, onPending, onIgnored }) {
  ...
  const handleReportSuccessfully = async () => {
    ...
    try {
      ...
      console.error('❌ Error reporting Instagram profile:', error);
        ...
        <div className="p-6">
          ...
          <div className="text-center py-8">
            ...
            <>
              ...
              {/* Profile Stats - Made more compact */}
              <div className="flex justify-evenly gap-4 text-sm text-gray-300 mb-3 p-3 bg-gray-800/30 rounded-lg">
                ...
                <div className="text-center">
                  <strong className="text-white text-md block">{profile.postsCount?.toLocaleString() || 0}</strong>
                  <span className="text-gray-400 text-xs">Posts</span>
                </div>
              </div>

              {/* Biography - Conditional with max height */}
              {profile.biography && (
                <div className="mb-3">
